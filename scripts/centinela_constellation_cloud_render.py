...
def generate(output_dir: Path) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="centinela-constellation-") as temporary:
        root = Path(temporary)
        ...
        try:
            result = FFmpegSceneRenderer(ffmpeg, ffprobe).render(
                plan, task_id="constellation-cloud-render", keep_segments=False
            )
        finally:
        ...
        shutil.copy2(video, output_dir / "constellation-preview.mp4")
        shutil.copy2(manifest_path, output_dir / "render-manifest.json")
        (output_dir / "ffprobe.json").write_text(json.dumps(probe, indent=2, sort_keys=True), encoding="utf-8")
        ...
        (output_dir / "media-resolution-report.json").write_text(
            json.dumps(outcome.report.model_dump(mode="json"), ensure_ascii=False, indent=2, sort_keys=True),
            encoding="utf-8",
        )

        selections = outcome.selection["selections"]
        required = {"2": "constellation_figure", "3": "orion_belt"}
        ...
        evidence = {
            ...
            "provenance_contract_pass": all(row["selected_publication_eligible"] is True for row in selections),
            "scientific_scope": "STATIC_CONSTELLATION_VISUAL_CLASSES_NO_COORDINATES_OR_EPHEMERIS",
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path("cloud-constellation-render"))
