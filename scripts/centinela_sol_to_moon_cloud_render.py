...
def generate(output_dir: Path) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="centinela-sol-to-moon-") as temporary:
        root = Path(temporary)
        ...
        if not outcome.report.publication_ready:
            raise AssertionError("MEDIA replay is not publication-ready")
        ...
        try:
            result = FFmpegSceneRenderer(ffmpeg, ffprobe).render(
                video_plan,
                task_id="sol-to-moon-cloud-render",
                keep_segments=False,
            )
        finally:
        ...
        assert manifest["requested_codec"] == "libx264"
        assert manifest["effective_codec"] == "libx264"
        assert manifest["codec_fallback"] is False
        ...
        shutil.copy2(video_path, output_dir / "sol-to-moon-preview.mp4")
        ...
        evidence = {
            ...
            "provenance_contract_pass": True,
            "scientific_scope": "NO_EPHEMERIS_OR_NUMERIC_CLAIMS_IN_FIXTURE",
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("cloud-sol-to-moon-render"),
    )
