...
def _build_media(media_root: Path) -> None:
    from test.services.test_planetary_media_cloud_replay import (
        _FIXTURES,
        _fixture_image,
        _write_sidecar,
    )
...
def generate(output_dir: Path) -> dict:
    ...
    from test.services.test_planetary_media_cloud_replay import _plan
    ...
    with tempfile.TemporaryDirectory(prefix="centinela-planetary-") as temporary:
        root = Path(temporary)
        ...
        if not outcome.report.publication_ready:
            raise AssertionError("PLANETARY MEDIA replay is not publication-ready")
        ...
        try:
            result = FFmpegSceneRenderer(ffmpeg, ffprobe).render(
                video_plan,
                task_id="planetary-cloud-render",
                keep_segments=False,
            )
        finally:
        ...
        shutil.copy2(video_path, output_dir / "planetary-preview.mp4")
        ...
        evidence = {
            ...
            "scientific_scope": "STATIC_PLANETARY_VISUAL_CLASSES_NO_EPHEMERIS_OR_NUMERIC_CLAIMS",
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path("cloud-planetary-render"))
