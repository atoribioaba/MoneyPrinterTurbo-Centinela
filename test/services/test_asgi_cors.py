...
class TestASGICORS(unittest.TestCase):
    """Verify secure browser-origin defaults and explicit compatibility modes."""
    ...
    def test_same_origin_and_server_clients_remain_compatible(self):
        client = self._create_client([])
        same_origin = client.get("/probe", headers={"Origin": "http://testserver"})
    ...
    def test_untrusted_multipart_request_is_rejected_before_side_effect(self):
        ...
        response = client.post(
            ...
            files={"file": ("attack.mp3", b"attacker-controlled", "audio/mpeg")},
        )
        self.assertEqual(response.status_code, 403)
