...
def bind(**kw):
    d=dict(recommendation_id="rec-1",parameter="intensity_bias",baseline_value=0.0,candidate_value=0.05,human_mapping_confirmed=True)
    d.update(kw)
    return PolicyBinding(**d)
...
def test_unsupported_rejected():
    with pytest.raises(RuntimeError):
        build_policy_candidate(PolicyCandidateRequest(recommendations=gate(),bindings=[bind(parameter="invented")]))
def test_range_enforced():
    with pytest.raises(RuntimeError):
        build_policy_candidate(PolicyCandidateRequest(recommendations=gate(),bindings=[bind(candidate_value=.5)]))
