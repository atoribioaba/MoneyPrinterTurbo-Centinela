...
class StrictPolicyCandidateModel(BaseModel):
...
class PolicyTargetComponent(str, Enum):
    CINEMATIC_DIRECTOR_REQUEST="CINEMATIC_DIRECTOR_REQUEST"
class PolicyBinding(StrictPolicyCandidateModel):
    recommendation_id:str=Field(min_length=1,max_length=128)
    target_component:PolicyTargetComponent=PolicyTargetComponent.CINEMATIC_DIRECTOR_REQUEST
    parameter:str=Field(min_length=1,max_length=128)
    baseline_value:PolicyScalar
    candidate_value:PolicyScalar
    human_mapping_confirmed:bool=False
class PolicyCandidateRequest(StrictPolicyCandidateModel):
    recommendations:EvidenceRecommendationGatePlan
    bindings:list[PolicyBinding]=Field(default_factory=list)
class CandidatePolicy(StrictPolicyCandidateModel):
    ...
    baseline_value:PolicyScalar
    candidate_value:PolicyScalar
    human_mapping_confirmed:bool=True
    requires_simulation:bool=True
    approved_for_activation:bool=False
    @model_validator(mode="after")
    def valid(self):
        if not self.human_mapping_confirmed or not self.requires_simulation or self.approved_for_activation:
            raise ValueError("F41 candidate guardrail violation")
        return self
class PolicyCandidatePlan(StrictPolicyCandidateModel):
    version:str=POLICY_CANDIDATE_VERSION
    source_recommendation_gate_hash:str
    deterministic:bool=True
    planning_only:bool=True
    resource_class:str="LIGHT"
    inferred_bindings:bool=False
    ...
    def valid(self):
        ...
        if not self.planning_only or self.inferred_bindings or self.edits_project or self.updates_director_policy or self.activates_policy or self.uses_llm or self.network_calls or self.database_writes or self.auto_publication:
            raise ValueError("F41 guardrail violation")
        return self
