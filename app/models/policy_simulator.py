...
class StrictPolicySimulatorModel(BaseModel):
...
class PolicySimulationCase(StrictPolicySimulatorModel):
    case_id:str=Field(min_length=1,max_length=128)
    plan:AstronomyVideoPlan
    video_base:VideoBasePlan
class PolicySimulatorRequest(StrictPolicySimulatorModel):
...
class PolicySimulationResult(StrictPolicySimulatorModel):
    ...
    baseline_tension_curve:list[float]
    candidate_tension_curve:list[float]
    baseline_climax_scene:int
    candidate_climax_scene:int
...
class PolicySimulatorPlan(StrictPolicySimulatorModel):
    version:str=POLICY_SIMULATOR_VERSION
    ...
    status:PolicySimulatorStatus
    case_count:int=Field(ge=0)
    simulation_count:int=Field(ge=0)
    behavior_change_count:int=Field(ge=0)
    results:list[PolicySimulationResult]
    policy_simulator_hash:str
    generated_at_utc:datetime
    @model_validator(mode="after")
    def valid(self):
        if self.simulation_count!=len(self.results) or self.behavior_change_count!=sum(x.behavior_changed for x in self.results):
            raise ValueError("simulation count mismatch")
        ...
        if not self.planning_only or not self.uses_real_cinematic_director or self.renders_video or self.gpu_required or self.uses_llm or self.network_calls or self.writes_runtime_config or self.activates_policy or self.auto_publication:
            raise ValueError("F42 guardrail violation")
        return self
