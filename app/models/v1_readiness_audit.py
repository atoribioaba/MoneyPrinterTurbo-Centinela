...
class V1ReadinessStatus(str, Enum):
    NOT_READY_FOR_ARCHITECTURE_FREEZE = "NOT_READY_FOR_ARCHITECTURE_FREEZE"
    READY_FOR_HUMAN_FREEZE_APPROVAL = "READY_FOR_HUMAN_FREEZE_APPROVAL"
...
class V1ReadinessCheck(StrictV1AuditModel):
    check_id: str
    passed: bool
    blocking: bool = True
    detail: str
...
class V1ReadinessAuditPlan(StrictV1AuditModel):
    version: str = V1_READINESS_AUDIT_VERSION
    deterministic: bool = True
    audit_only: bool = True
    final_phase: bool = True
    resource_class: str = "LIGHT"
    architecture_v1_frozen: bool = False
    ...
    oss_audit_count: int = Field(ge=0)
    oss_audit_verified_count: int = Field(ge=0)
    oss_audit: list[OSSAuditEntry]
    v1_readiness_hash: str
    ...
    def validate_plan(self):
        ...
        if self.oss_audit_count != len(self.oss_audit) or (
            self.oss_audit_verified_count
            != sum(item.verified for item in self.oss_audit)
        ):
            raise ValueError("OSS audit count mismatch")
        if self.architecture_v1_frozen or self.freeze_executed:
            raise ValueError("F58 may authorize freeze but never execute it")
        if (
            not self.audit_only
            or not self.final_phase
            or self.auto_publication
            or self.auto_activation
