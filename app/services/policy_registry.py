...
def build_policy_registry(request:PolicyRegistryRequest)->PolicyRegistryPlan:
    candidates={x.policy_candidate_id:x for x in request.candidates.candidates}
    approvals={x.policy_candidate_id:x for x in request.approvals.records if x.decision==HumanDecision.APPROVE}
    prev={}
    seen=set()
    out=[]
    for x in request.previous_versions:
        ...
        if k in prev:
            raise PolicyRegistryError("duplicate previous policy reference")
        prev[k]=x.policy_version
    for cid in sorted(approvals):
        a=approvals[cid]
        c=candidates.get(cid)
        if c is None:
            raise PolicyRegistryError("approved policy candidate not found")
        k=(c.target_component,c.parameter)
        if k in seen:
            raise PolicyRegistryError("multiple approved candidates target same parameter")
        seen.add(k)
        pv=prev.get(k)
        stable={"candidate":cid,"approval":a.approval_record_hash,"target":c.target_component.value,"parameter":c.parameter,"value":c.candidate_value,"previous":pv}
