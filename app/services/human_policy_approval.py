...
class HumanPolicyApprovalError(RuntimeError):
    pass
def _hash(v):
...
def build_human_policy_approval(request:HumanPolicyApprovalRequest)->HumanPolicyApprovalPlan:
    safe={x.policy_candidate_id:x for x in request.comparator.comparisons if x.safe_for_human_review}
    seen=set()
    records=[]
    for d in request.decisions:
        if d.policy_candidate_id in seen:
            raise HumanPolicyApprovalError("duplicate human decision")
        seen.add(d.policy_candidate_id)
        if d.policy_candidate_id not in safe:
            raise HumanPolicyApprovalError("human decision targets a candidate not safe for review")
    ...
    records.sort(key=lambda x:x.policy_candidate_id)
    stable={"version":HUMAN_POLICY_APPROVAL_VERSION,"comparator":request.comparator.policy_comparator_hash,"records":[x.model_dump(mode="json") for x in records]}
