...
class PolicyCandidateError(RuntimeError): pass
...
def _check(p,v):
    ...
    if spec is None: raise PolicyCandidateError(f"unsupported policy parameter: {p}")
    kind,lo,hi=spec
    if kind=="bool" and type(v) is not bool: raise PolicyCandidateError(f"{p} requires boolean values")
    if kind=="float":
        if type(v) is bool or not isinstance(v,float): raise PolicyCandidateError(f"{p} requires strict float values")
        if not lo<=v<=hi: raise PolicyCandidateError(f"{p} outside allowed range [{lo}, {hi}]")
def build_policy_candidate(request:PolicyCandidateRequest)->PolicyCandidatePlan:
    recs={x.recommendation_id:x for x in request.recommendations.recommendations}; seen=set(); out=[]
    for b in request.bindings:
        if not b.human_mapping_confirmed: continue
        if b.recommendation_id in seen: raise PolicyCandidateError("duplicate recommendation binding")
        seen.add(b.recommendation_id); r=recs.get(b.recommendation_id)
        if r is None: raise PolicyCandidateError(f"unknown recommendation_id: {b.recommendation_id}")
        _check(b.parameter,b.baseline_value); _check(b.parameter,b.candidate_value)
        if b.baseline_value==b.candidate_value: raise PolicyCandidateError("baseline and candidate values must differ")
    ...
    out.sort(key=lambda x:x.policy_candidate_id); stable={"version":POLICY_CANDIDATE_VERSION,"source":request.recommendations.evidence_recommendation_gate_hash,"candidates":[x.model_dump(mode="json") for x in out]}
