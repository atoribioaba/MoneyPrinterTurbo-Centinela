...
def build_operational_hardening(request:OperationalHardeningRequest)->OperationalHardeningPlan:
    ...
    for name,ok in required.items():
        if not ok:
            add(f"missing_{name}",FindingSeverity.BLOCK,f"required capability missing: {name}")
    if s.free_space_gb < 5:
        add("disk_critical",FindingSeverity.BLOCK,f"free_space_gb={s.free_space_gb:.2f}")
    elif s.free_space_gb < 20:
        add("disk_low",FindingSeverity.WARN,f"free_space_gb={s.free_space_gb:.2f}")
    if s.backup_bundle_count==0:
        add("no_checkpoint_bundles",FindingSeverity.WARN,"no checkpoint bundles detected")
    if s.git_network_workaround_required:
        add("git_network_workaround",FindingSeverity.WARN,"normal Windows resolver/Schannel path is not trusted; forced GitHub IP + OpenSSL workaround required")
    if s.plaintext_secret_config_count>0:
        add("plaintext_secret_config",FindingSeverity.WARN,f"secret-bearing config keys with non-empty values={s.plaintext_secret_config_count}; values are not emitted")
    blocks=sum(x.severity==FindingSeverity.BLOCK for x in f)
    warns=sum(x.severity==FindingSeverity.WARN for x in f)
