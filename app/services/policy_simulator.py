...
def _validate_candidate(candidate):
    ...
    if spec is None:
        raise PolicySimulatorError(f"unsupported policy parameter: {candidate.parameter}")
    kind,lo,hi=spec
    value=candidate.candidate_value
    if kind=="bool" and type(value) is not bool:
        raise PolicySimulatorError(f"{candidate.parameter} requires boolean candidate value")
    if kind=="float":
        if type(value) is bool or not isinstance(value,float):
            raise PolicySimulatorError(f"{candidate.parameter} requires float candidate value")
        if not lo<=value<=hi:
            raise PolicySimulatorError(f"{candidate.parameter} outside allowed range [{lo}, {hi}]")
def _hash(v):
    return hashlib.sha256(json.dumps(v,sort_keys=True,separators=(",",":"),default=str).encode()).hexdigest().upper()
def _checks(d):
    c=d.structural_checks
    return all((c.act_order_valid,c.climax_present,c.epilogue_present,c.scene_number_alignment,c.duration_alignment,c.placeholders_preserved))
def build_policy_simulator(request:PolicySimulatorRequest)->PolicySimulatorPlan:
    ...
    for candidate in request.candidates.candidates:
        if candidate.target_component!=PolicyTargetComponent.CINEMATIC_DIRECTOR_REQUEST:
            raise PolicySimulatorError("unsupported target component")
        ...
        for case in request.cases:
            ...
            if actual!=candidate.baseline_value:
                raise PolicySimulatorError(f"baseline mismatch for {candidate.parameter}: request={actual!r} candidate={candidate.baseline_value!r}")
            ...
            base=director.build(base_req)
            cand=director.build(cand_req)
    ...
    out.sort(key=lambda x:(x.policy_candidate_id,x.case_id))
    stable={"version":POLICY_SIMULATOR_VERSION,"candidate_hash":request.candidates.policy_candidate_hash,"results":[x.model_dump(mode="json") for x in out]}
