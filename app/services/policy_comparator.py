...
def build_policy_comparator(request:PolicyComparatorRequest)->PolicyComparatorPlan:
    g=defaultdict(list)
    for r in request.simulations.results:g[r.policy_candidate_id].append(r)
    out=[]
    for cid in sorted(g):
        rows=g[cid]; sr=sum((not x.candidate_structural_checks_pass) or (not x.baseline_structural_checks_pass) for x in rows); pr=sum(not x.placeholders_preserved for x in rows)
    ...
    safe=sum(x.safe_for_human_review for x in out); status=PolicyComparatorStatus.WAITING_FOR_SIMULATIONS if not out else (PolicyComparatorStatus.SAFE_CANDIDATES_READY if safe else PolicyComparatorStatus.NO_SAFE_CANDIDATES)
